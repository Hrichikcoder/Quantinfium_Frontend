...
export const LoginForm = ({ onSuccess }: LoginFormProps) => {
  ...
  const { syncAuthFromLocalStorage } = useAuth();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      // Pass email as username since that's what the API expects
      await apiLogin(data.email, data.passwordOrPin);
      // Reflect token-based auth in context so Navbar updates immediately
      ...
      const message = typeof e === "string" ? e : "Login failed. Please try again.";
      setError(message);
    } finally {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <div className="text-right mt-1">
            <Link to="/forgot-password" className="text-xs text-green-200 hover:underline">Forgot password?</Link>
          </div>
        </div>
